...
| 2 | Tier 1, Node, `npm ci` in `verify/`, Chromium | The selftests of the DOM, style and pixel tools, the search consistency check and the relevance cases of `verify/fixtures/demo/search-relevance.json`, the search dialog in Chromium, Firefox and WebKit under a subpath, the lucide oracle, the agent score of the demo served by the `pholio dev` router | `node verify/run.mjs --tier 2` |
